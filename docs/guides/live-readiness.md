...
Default private setup file status on the current Mac install: doctor --live reports `34 passing, 19 blocked` until the operator-owned inputs are real and verified:
The current live unlock checklist is Provider accounts: 9 blockers, Internal credential: 2 blockers, and V1 evidence: 8 blockers.
Most remaining blockers currently report `needs real values`, `mismatch`, or `missing`.

| Area | Current status |
| --- | --- |
| Model providers | Real Anthropic/OpenRouter keys, private OpenAI-compatible endpoint values, positive private context window, refreshed OpenRouter profile metadata, and successful provider smoke calls are still required. |
| Internal credential smoke | The internal API health URL still needs a real value, and the encrypted credential smoke must still succeed. |
| V1 evidence manifest | Real multi-day goal evidence, provider and credential smoke transcripts, Dream artifacts, public contribution evidence, published canonical-world artifacts, curation stats, and the required two-week follow-up evidence are still required. |
