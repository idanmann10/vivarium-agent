...
| Missing Bun recovery is copyable | `scripts/install.test.ts` covers the missing-Bun dependency path and requires the installer to print `curl -fsSL https://bun.sh/install \| bash` plus the instruction to reload the shell and rerun the Vivarium installer | Complete |
