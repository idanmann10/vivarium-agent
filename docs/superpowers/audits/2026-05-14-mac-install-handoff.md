...
| Invalid LaunchAgent ports fail before install work | `scripts/install.test.ts` covers the `VIVARIUM_DAEMON=launchd` path with an invalid `VIVARIUM_DAEMON_PORT` and requires the installer to print the invalid value, require an integer from 1 to 65535, and stop before clone/update work | Complete |
