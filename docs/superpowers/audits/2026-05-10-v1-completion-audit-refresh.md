...
| --- | --- | --- |
| Resumed objective: check current Claude Managed Agents and Claude Code agent docs before future agent-building work | `docs/reference/claude-agent-formats.md` records current Claude Managed Agents concepts, API beta/toolset details, agent fields, skill source shapes, MCP secret split, Claude Code subagent YAML/frontmatter fields, scope priority, worktree isolation, and agent-team reuse of subagent definitions. `scripts/reference-docs.test.ts` guards the reference, and the 2026-05-11 resume section below lists the official docs checked | Complete locally |
...
| V1 done: five real goals over a week and two-week measurable improvement | Starter-pack and world-subscription evidence is configured; provider smokes, internal credential smoke, five dated real coding goals spanning a week, behavior-loop evidence, Dream artifacts, public contribution/auto-merge/other-agent pull-use evidence, published artifact reads, curation stats, and the fourteen-day-or-later follow-up remain missing in fresh `doctor --live` output | Incomplete externally |
...
- The ignored local env file now points `VIVARIUM_V1_EVIDENCE_PATH` at `/Users/idanmann/.codex/memories/vivarium-v1-evidence.json` so the partially filled, non-secret evidence manifest is durable across shell cleanup; it currently contains verified canonical/private world subscription refs plus local starter-pack init/run evidence. It also points `VIVARIUM_WORLD_SUBSCRIPTIONS_PATH` at `/Users/idanmann/.codex/memories/vivarium-world-subscriptions.json`, a non-secret durable copy of the canonical/private world subscription registry.
