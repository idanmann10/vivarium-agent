...
| Phase 1 read-only world paths | Local reader, retrieval, multi-world search, proposed anti-pattern/trace/run search, published-run search, file-backed world subscription registry, injectable git clone/update pull tests, and a pull-then-search transmission smoke helper exist; CLI routes `world pull`, repeated-root `world search`, saved-subscription `world search --subscriptions-path`, `world subscribe`, `world subscriptions`, and `world transmission-smoke` against local git remotes | Complete locally |
...
- `bun test packages/world/src/pull.test.ts apps/cli/src/commands/world.test.ts apps/cli/src/dispatcher.test.ts`: 25 tests passed, including local second-install world transmission verification, repeated-root CLI search routing, and saved subscription routing.
...
- `bun test apps/cli/src/commands/run.test.ts apps/cli/src/dispatcher.test.ts`: 20 tests passed, including configured provider-backed `run` routing, saved provider profile `run` routing, missing provider env behavior, unsupported provider-kind rejection, saved world subscription routing, and local command dispatch.
- `bun test apps/cli/src/dispatcher.test.ts`: 16 tests passed, including CLI parser routing for local commands, provider-backed `run`, provider profile configure/list/smoke/run routing, `doctor --live`, `providers smoke`, `github smoke`, guarded `github discussion`, guarded `github pull-request`, `github workflow-runs`, repeated-root `world search`, saved-subscription `world search`, `world transmission-smoke`, and `daemon smoke`.
- `bun test apps/cli/src/commands/credentials.test.ts apps/cli/src/dispatcher.test.ts`: 19 tests passed, including encrypted credential HTTP smoke, saved provider profile routing, saved world subscription routing, and missing-credential behavior.
...
- `bun test apps/cli/src/commands/world.test.ts apps/cli/src/dispatcher.test.ts`: 22 tests passed, including command and dispatcher multi-world search with source labels, saved world subscription registries, saved provider profile routing, and local world pull/transmission routing.
- `bun test packages/world/src/pull.test.ts apps/cli/src/commands/world.test.ts apps/cli/src/dispatcher.test.ts`: 25 tests passed, including injectable git clone/update, non-git destination rejection, `world pull`, repeated-root `world search`, saved-subscription `world search`, saved provider profile routing, `world transmission-smoke`, and CLI routing against a local git remote.
...
- `bun run test`: 161 tests passed, 0 failed.
- `bun run build`: 9 entrypoints present.

## Next Unblocked Local Work

The highest-value remaining gaps after the recorded local demo, world transmission-smoke CLI, provider-backed CLI run path, credential smoke path, proposed anti-pattern/trace/run retrieval, Node-side `better-sqlite3` verifier, and local Compose daemon execution are live external verification:
