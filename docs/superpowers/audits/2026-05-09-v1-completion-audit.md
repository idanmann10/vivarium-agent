...
- `bun test apps/cli/src/dispatcher.test.ts`: 13 tests passed, including CLI parser routing for local commands, `doctor --live`, `providers smoke`, `github smoke`, guarded `github discussion`, guarded `github pull-request`, `github workflow-runs`, repeated-root `world search`, `world transmission-smoke`, and `daemon smoke`.
