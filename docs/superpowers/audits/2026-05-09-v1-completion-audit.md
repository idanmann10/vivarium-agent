...
| Phase 1 read-only world paths | Local reader, retrieval, multi-world search, proposed anti-pattern/trace/run search, published-run search, injectable git clone/update pull tests, and a pull-then-search transmission smoke helper exist; CLI routes `world pull`, repeated-root `world search`, and `world transmission-smoke` against local git remotes | Complete locally |
...
| Phase 3 multi-world subscriptions | Multi-world retrieval tests cover skills and published runs with source labels; CLI tests cover repeated world roots with source labels; local-reader tests cover proposed anti-pattern, trace, and run discovery used by the same search path | Complete locally |
...
- `bun test apps/cli/src/commands/run.test.ts apps/cli/src/dispatcher.test.ts`: 17 tests passed, including configured provider-backed `run` routing, missing provider env behavior, unsupported provider-kind rejection, and local command dispatch.
- `bun test apps/cli/src/dispatcher.test.ts`: 14 tests passed, including CLI parser routing for local commands, provider-backed `run`, `doctor --live`, `providers smoke`, `github smoke`, guarded `github discussion`, guarded `github pull-request`, `github workflow-runs`, repeated-root `world search`, `world transmission-smoke`, and `daemon smoke`.
- `bun test apps/cli/src/commands/credentials.test.ts apps/cli/src/dispatcher.test.ts`: 17 tests passed, including encrypted credential HTTP smoke and missing-credential behavior.
...
- `bun test apps/cli/src/commands/world.test.ts apps/cli/src/dispatcher.test.ts`: 18 tests passed, including command and dispatcher multi-world search with source labels plus local world pull/transmission routing.
- `bun test packages/world/src/pull.test.ts apps/cli/src/commands/world.test.ts apps/cli/src/dispatcher.test.ts`: 20 tests passed, including injectable git clone/update, non-git destination rejection, `world pull`, repeated-root `world search`, `world transmission-smoke`, and CLI routing against a local git remote.
